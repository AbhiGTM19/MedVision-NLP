...
## 1. Target Components:
**Path:** `main.py`, `api/`, `schemas/`, `services/`
...
| **Pydantic Schemas** | `schemas/predict.py` |
| **Model Service** | `services/model_service.py` |
| **Shared Utilities** | `common.py`, `utils.py` |
...
1. Models are loaded eagerly via the singleton `model_service = ModelService()` in `services/model_service.py`. Do NOT add per-request model loading.
2. Update `SKILL_STATE.json` upon task completion.

## 5. Domain-Specific Rules:
- Follow `.agent/rules/03-fastapi-ml.md` constraints.
- Never load weights inside route handlers. The singleton in `services/model_service.py` handles all model initialization at import time.
...
- **To Integration QA:** Ensure `schemas/` align with `HANDOFF_SCHEMA.json`.

## 8. Troubleshooting Decision Tree:
- **Issue: Inference timeout** -> *Check:* `ModelService._load_models()` in `services/model_service.py` -> *Fix:* Ensure HF Hub download succeeds or model files exist locally.
