...
When operating within the backend logic (`api/`, `core/`, `schemas/`), strictly adhere to the following constraints:

## 1. Inference Server Reliability
- Models are loaded eagerly at startup via the singleton `ModelService()` in `services/model_service.py`. Never load model weights inside a route handler per-request.
...
- Track experiments meticulously using MLflow.
