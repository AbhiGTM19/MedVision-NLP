...
- Models are loaded eagerly at startup via the singleton `ModelService()` in `backend/services/model_service.py`. Never load model weights inside a route handler per-request.
