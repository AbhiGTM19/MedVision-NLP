...
- Models are loaded eagerly at startup via the singleton `ModelService()` in `backend/services/model_service.py` and `OCRService()` in `backend/services/ocr_service.py`. Never load model weights inside a route handler per-request.
...
- Maintain dual routes: `/predict` for Text (Bio_ClinicalBERT) and `/predict-image` for Image (EasyOCR + Fusion).
